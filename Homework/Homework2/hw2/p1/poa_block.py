...
class PoABlock(Block):
    ...
    def mine(self):
        ...
        while not self.seal_is_valid():
            signer = SigningKey.from_string(self.get_private_key(), curve=ecdsa.NIST192p)
            signature = signer.sign(self.unsealed_header().encode('utf-8'))
            sign_int = int.from_bytes(signature, byteorder='big')
            self.set_seal_data(sign_int)
