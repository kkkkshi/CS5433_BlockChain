...
class Block(ABC, persistent.Persistent):
    ...
    def is_valid(self):
        ...
        if self.is_genesis:
            if not (self.height == 0 and self.parent_hash == "genesis"):
        ...
        if not self.is_genesis:
            ...
            for tx in self.transactions:
                ...
                if (tx.hash in txs_in_block) or (
                nonempty_intersection(blocks_in_chain, chain.blocks_containing_tx.get(tx.hash, []))):
                ...
                for input_ref in tx.input_refs:
                    ...
                    input_idx = int(input_ref.split(":")[1])
                    ...
                    if not input_idx < len(candidate_tx.outputs):
                        return False, "Required output not found"
                    output = candidate_tx.outputs[input_idx]
                    ...
                    if user_transacting is None:
                        user_transacting = output.receiver
                    elif not output.receiver == user_transacting:
                    ...
                    if (input_ref in inputs_spent) or (
                    nonempty_intersection(blocks_in_chain, chain.blocks_spending_input.get(input_ref, []))):
                    ...
                    if (input_tx_hash in txs_in_block) or (
                    nonempty_intersection(blocks_in_chain, chain.blocks_containing_tx.get(input_tx_hash, []))):
